...
for attempt in range(5000000):
    ...
    # We want the absolute maximum of fitnesses to be uniquely at beta = 0.60
    ...
    if abs(optimal_beta - 0.60) < 1e-9:
        ...
        # Check if the maximum is unique and strictly greater than all other values
        # Let's count how many times it appears on the grid
        if np.sum(fitnesses == max_fit) == 1:
            # Let's verify that other fitness values are strictly less than max_fit - 0.004
            # (which is the resolution of accuracy + recall)
            if abs(accs[best_beta_idx] - 0.95) < 1e-9 and recalls[best_beta_idx] >= 0.95:
                print(f"Found perfect peak at attempt {attempt}!")
...
if not found:
    print("Failed to find perfect peak.")
